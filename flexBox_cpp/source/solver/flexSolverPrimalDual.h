...
public:
	...
	{
		// reset
		for (auto &v : data->sigmaElt)
			std::fill(v.begin(), v.end(), 0);
		for (auto &v : data->tauElt)
			std::fill(v.begin(), v.end(), 0);

		// global estimates per operator
		for (int k = 0; k < (int)termsDual.size(); ++k)
		{
			...
			for (int i = 0; i < numDuals; ++i)
			{
				...
				for (int j = 0; j < numPrimals; ++j)
				{
					...
					//reduction
					T maxF = static_cast<T>(0);
					T maxT = static_cast<T>(0);

					for (int r = 0; r < (int)rowF.size(); ++r)
					{
						maxF = std::max(maxF, rowF[r]);
						maxT = std::max(maxT, rowT[r]);
					}

					//accumulate (SAFE upper bound)
					for (int r = 0; r < (int)data->sigmaElt[dualNum].size(); ++r)
						data->sigmaElt[dualNum][r] += maxF;

					for (int r = 0; r < (int)data->tauElt[primalNum].size(); ++r)
						data->tauElt[primalNum][r] += maxT;
				}
			}
		}
