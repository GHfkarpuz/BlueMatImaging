...
for i in range(num_iterations):
    ...
    # initialise solver
    solver = FlexBoxSolver(maxIt=200, tol=1e-6, verbose=1)
    ...
    for j in range(len(frames)-1):
        ...
        solver.add_dual(
                ...
                operator_dict = [weightedGradientOp1, weightedGradientOp2, negIdentityOp]
            )
    ...
    solver.add_dual(
            ...
            operator_dict = [weightedGradientOp1, weightedGradientOp2, identityOp]
        )
    ...
    for j in range(len(frames)-1):
        if((i+1)%5==0):
            ...
            plt.savefig(f"resultL2TVOpticalFlowBlueMatExample{i+1},{j+1}.png", dpi=200, bbox_inches="tight")
