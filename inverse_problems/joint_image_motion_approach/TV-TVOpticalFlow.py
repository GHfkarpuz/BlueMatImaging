...
while True:
    ...
    if idx in target_set:
        ...
        frame_gray = cv2.resize(frame_gray, None, fx=0.9, fy=0.9)
...
weight_2 = 0.1 #regularizer for u
...
motionField_0 = [np.ones(nPx)*0.1 for i in range(len(frames)-2)]
motionField_1 = [np.ones(nPx)*0.1 for i in range(len(frames)-2)]
...
for i in range(num_iterations):
    ...
    for j in range(0,len(frames)-2):
        ...
        solver.add_dual(
                ...
                operator_dict = [weightedGradientOp1, weightedGradientOp2, halfNegIdentityOp, halfIdentityOp]
            )
        ...
        solver.add_dual(
                ...
                operator_dict = [weightedGradientOp1, weightedGradientOp2, negIdentityOp, identityOp]
            )
        """
    """
    print("1")
    ...
    solver.add_dual(
            ...
            operator_dict = [weightedGradientOp1, weightedGradientOp2, identityOp, negIdentityOp]
        )
    print("1")"""
    ...
    for j in range(len(frames)-2):
        if((i+1)%1==0):
            ...
            plt.savefig(f"resultTVTVOpticalFlowBlueMatExample{i+1},{j+1}.png", dpi=200, bbox_inches="tight")
